...
@php
    ...
    $clusterData = [
        'T' => Assessment::where('status', 'completed')->whereJsonContains('scores->T', '>', 2)->count(),
        'C' => Assessment::where('status', 'completed')->whereJsonContains('scores->C', '>', 2)->count(),
        'B' => Assessment::where('status', 'completed')->whereJsonContains('scores->B', '>', 2)->count(),
        'S' => Assessment::where('status', 'completed')->whereJsonContains('scores->S', '>', 2)->count(),
        'F' => Assessment::where('status', 'completed')->whereJsonContains('scores->F', '>', 2)->count(),
    ];
@endphp
        ...
        <div class="max-w-6xl mx-auto flex flex-col lg:flex-row justify-between items-center relative z-10 gap-8">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <span class="inline-block mt-1 bg-yellow-400 text-black text-xs font-bold px-3 py-1 rounded-full">Admin</span>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="max-w-6xl mx-auto">
                        ...
                        @foreach($clusterData as $cluster => $count)
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-3">
                                    <span class="w-8 h-8 bg-{{ $clusterColors[$cluster] }}-500 rounded-full flex items-center justify-center text-white text-sm font-bold">
                                        {{ $cluster }}
                                    </span>
                                    <div>
                                ...
                                <div class="text-right">
                                    ...
                                    <div class="w-20 bg-gray-200 rounded-full h-2">
                                        <div class="bg-{{ $clusterColors[$cluster] }}-500 h-2 rounded-full" style="width: {{ $percentage }}%"></div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="bg-white rounded-2xl p-6 shadow-md">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                            ...
                            <span class="bg-blue-100 text-blue-800 text-sm font-bold px-2 py-1 rounded">{{ $totalAssessments }}</span>
                        </div>

                        <div class="pt-4 border-t border-gray-200">
                            <div class="grid grid-cols-2 gap-4">
                                <a href="{{ route('assessment.index') }}" class="bg-teal-500 text-white px-4 py-3 rounded-lg font-semibold hover:bg-teal-600 transition-colors text-center">
                                    View Assessment
                                </a>
                                <button class="border border-gray-300 text-gray-600 px-4 py-3 rounded-lg font-semibold hover:bg-gray-50 transition-colors">
                                    Export Data
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="space-y-4">
                        @foreach($popularPathways as $index => $pathway)
                            <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                                <div class="flex-shrink-0 w-8 h-8 bg-teal-500 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                    {{ $index + 1 }}
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-semibold text-teal-700">{{ $pathway->name }}</h4>
                                    <p class="text-sm text-gray-600">{{ $pathway->category }} • {{ $pathway->difficulty_level }}</p>
                                </div>
                                <div class="text-right">
                            ...
                            <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-gray-600">
                                        Completed assessment • {{ $assessment->completed_at ? $assessment->completed_at->diffForHumans() : 'Recently' }}
                                    </p>
                                    @if($assessment->results->count() > 0)
            ...
            <div class="bg-white rounded-2xl p-6 shadow-md mt-8">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <a href="{{ route('assessment.index') }}" class="flex flex-col items-center p-4 bg-teal-50 rounded-lg hover:bg-teal-100 transition-colors">
                        <div class="w-12 h-12 bg-teal-500 rounded-full flex items-center justify-center text-white mb-3">
